...
final_cali <- clean_avocado |> 
  right_join(regions_cali, by = "region") |>
...
```{r wd-4-factor}
#lab 5 Capture over the week Q3 new correct answer
...
ggplot(data= rodents, 
       aes(x = week_day_level, y = captured_rodents, fill = day_of_week)) +
...
```{r wd-6-anti}
#Lab 4 Q5 (revised, slice_max, inner_join)
...
metro_regions <- clean_avocado |> 
  ...
  inner_join(clean_avocado, by = "region")

ggplot(data = metro_regions, 
...
```{r r-3-1}
#this will come from lab 7 with the functions
```
...
```{r dvs-1-num}
#Lab 2 Q4 (revised, added units in title)
 ggplot(data = surveys, 
...
#Lab 2 Challenge 
ggplot(data = surveys, 
       mapping = aes(x = weight, y = species)) +           geom_density_ridges() + 
       labs(x = "Weight", 
            y = " ", 

...
ggplot(data= summarized_data, aes(x = year, y = weight, color = genus)) +
  geom_line() +
  theme_classic() +
...
```{r wd-6-anti}
#Lab4 Q5 revised arrange desc into slice_max
...
ggplot(data= rodents, 
       aes(x = reorder(day_of_week, captured_rodents), y = captured_rodents, fill = captured_rodents)) +
...
ggplot(data = surveys, mapping = aes(x=species, y=weight)) + 
  geom_boxplot(outlier.shape=NA) + geom_jitter(col="tomato", alpha=0.1) +
  labs(x="Species", y="Weight", title = "Distribution of weight within each Species")
```
...
```{r dvs-5-2}
#Lab 4 Q6 (not sure if too many objects here and if it relates to question???)

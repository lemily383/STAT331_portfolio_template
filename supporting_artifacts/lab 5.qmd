...
ggplot(data= summarized_mean, aes(x = year, 
  y = mean_weight, color = genus)) +
  geom_line() +
  theme_classic() +
  scale_x_continuous(limits = c(1977, 2002)) +
  scale_color_discrete(guide = guide_legend(title = "Genus")) +
...
ggplot(data= summarized_mean, 
       mapping = aes(x = year, 
                     y = mean_weight, 
                     color = fct_reorder(genus, desc(mean_weight)))) +
  geom_line() +
  theme_classic() +
  scale_x_continuous(limits = c(1977, 2002)) +
  scale_color_discrete(guide = guide_legend(title = "Genus")) +
...
I wanted to make this visualization more visually appealing and easier to distinguish between the large numbers so I thought adding a gradient aspect to it would be helpful. I learned about this function through https://ggplot2-book.org/scale-colour.html
...
I learned to use the order() function in R through https://www.datacamp.com/tutorial/sorting-in-r
...
ggplot(data= rodents, 
       aes(x = week_day_level, y = captured_rodents, fill = captured_rodents)) +
  geom_bar(stat = "identity") +
  labs(x = "Time of Week", 
       y = "", 
